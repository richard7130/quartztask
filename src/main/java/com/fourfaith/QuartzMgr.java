...
public class QuartzMgr {
     ...
     */
    public static void addAfterJob(Job job, int afterSencond) {
        try {
     ...
     * @throws SchedulerException
     */
    public static void addEvaJob(Job job, int evaSenond) {
        try {
            //创建任务
            String uuid = CommonUtils.getGuid();
            JobDetail jobDetail = JobBuilder.newJob(job.getClass()).withIdentity(job.getClass().getName(), uuid).build();
     ...
     * @throws SchedulerException
     */
    public static void addAfterEvaJob(Job job, int afterSencond, int evaSenond) {
        try {
            Date startTime = DateBuilder.futureDate(afterSencond, DateBuilder.IntervalUnit.SECOND);
            String uuid = CommonUtils.getGuid();
            //创建任务
     ...
     */
    public static void addTimingJob(Job job, String cron) {
        try {
            String uuid = CommonUtils.getGuid();
            //创建任务
            JobDetail jobDetail = JobBuilder.newJob(job.getClass()).withIdentity(job.getClass().getName(), uuid).build();
